import {CHECK_NODE_STATUS_START, CHECK_NODE_STATUS_SUCCESS, CHECK_NODE_STATUS_FAILURE,SET_BLOCKS_FROM_NODE,SET_BLOCKS_FROM_NODE_FAILURE} from '../constants/actionTypes';
...
export default function nodesReducer(state = initialState().nodes, action) {
  ...
  switch (action.type) {
    ...
    case SET_BLOCKS_FROM_NODE_FAILURE:
      list = state.list;
      nodeIndex = state.list.findIndex(p => p.url === action.node.url);
      if (nodeIndex >= 0) {
        list = [
